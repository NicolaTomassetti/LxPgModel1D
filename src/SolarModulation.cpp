...
void SolarModulation::InitModulation(){
    ...
    if( fabs(ThisXiD)< 1.e-6 ) ThisXiD=0.;
...
void SolarModulation::InitJLis(){
  ...
  if( kLISModel>0){
    ...
    if( kLISModel==1) inLIS= new TFile("$NTBASEDIR/THEORY/00matisse/PHeModel/input/grLIS_pHe_Tomassetti_ApJ_2017.root","READ");
    
    // Mod2 NT PRD 2017 
    if( kLISModel==2) inLIS= new TFile("$NTBASEDIR/THEORY/00matisse/PHeModel/input/grLIS_pHe_Tomassetti_PRD_2017.root","READ");
    
    // Mod3 NT ASR 2017 
    if( kLISModel==3) inLIS= new TFile("$NTBASEDIR/THEORY/00matisse/PHeModel/input/grLIS_pHe_Tomassetti_ASR_2017.root","READ");
...
void SolarModulation::ForceFieldSolution(){
  ...
  ForceField->SetParameter(0,ThisPhi); // Phi from KScale
...
void SolarModulation::Solve(){
  ...
  for(int i=0; i<np; ++i){ //loop on rigidity p 
    ...
    f[0][i] = 0.; 
    f[nr-1][i] = (JLisT->Eval(T)/(p*p*NTF))/f0; 
...
void SolarModulation::SetSolutions(){
  ...
  cout<<"Reference flux at radius R="<<radius[indR0]/AU<<" AU"<<endl;
...
void SolarModulation::StoreResults(){
  ...
  if(DRIFT==0){ 
    ...
    outFile= new TFile(Form("$NTBASEDIR/THEORY/00matisse/OUT/PHeModel_DriftOFF/hrun_BetaOFF_DriftOFF_FluxResults_LIS%d_Z%d_A%d_RunID%d.root",kLISModel,zz,aa,RunID),"RECREATE");
  }
  ...
  if(DRIFT==1){ // kk and dd indices
    ...
    outFile= new TFile(Form("$NTBASEDIR/THEORY/00matisse/OUT/PHeModel_Z%d_A%d/hrun_FluxResults_LIS%d_Z%d_A%d_XiD%d_K%d_RunID%d.root",zz,aa,kLISModel,zz,aa,dd,kk,RunID),"RECREATE");
  }
...
void SolarModulation::PlotSolution(){
  ...
  for(int ee=0;ee<16;ee++){
    ...
    TFile* inFileProton= new TFile("$NTBASEDIR/THEORY/00matisse/PHeModel/data/grProtonData.root","READ");
    inFileProton->cd();
